...
# First run `uv run ../gsm8k/gsm8k_dataset.py --output_dir $HOME/data/gsm8k` to setup the dataset.
...
SKYRL_PYTHONPATH_EXPORT=1 uv run --directory "$PROJECT_ROOT" --isolated --extra vllm0p12 \
    ...
    trainer.project_name="ray_serve_llm_engine" \
    trainer.run_name="ray_serve_llm_engine_test" \
